function Cs = cov4D_original(WAVEVECTOR_DATA,EIGENVALUE_DATA)
    [N_wv,N_eig,N_struct] = size(EIGENVALUE_DATA);
    N_k = sqrt(N_wv);
    original_domain_X = linspace(-pi,pi,N_k);
    original_domain_Y = linspace(0,pi,N_k);
    C4D = zeros(length(original_domain_X),length(original_domain_Y),length(original_domain_X),length(original_domain_Y));
    for eig_idx = 1:N_eig
