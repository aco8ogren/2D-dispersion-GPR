...
eig_idxs = [1 2 5 10 15];
struct_idxs = 1:100;
model_names = {'GPR'}; % {'GPR','linear','nearest','cubic','makima','spline'};
num_sample_points = [5 10 15 20 25 50 75 100 150 200 250 300 350 400];
...
% sampling_order_path = ['C:\Users\alex\OneDrive - California Institute of Technology\Documents\Graduate\Research\2D-dispersion-GPR\'...
%     'sampling_order_information_up_to_128.mat'];

% sampling_order_path = ['C:\Users\alex\OneDrive - California Institute of Technology\Documents\Graduate\Research\2D-dispersion-GPR\'...
%     'sampling_order_information_up_to_1326.mat'];

sampling_order_path = ['C:\Users\alex\OneDrive - California Institute of Technology\Documents\Graduate\Research\2D-dispersion-GPR\'...
    'sampling_order_information_newest.mat'];

sampling_order_data = load(sampling_order_path);
N_sample_max = size(sampling_order_data.sampling_orders,1); % kind of correct?
...
err_L2 = zeros(length(eig_idxs),length(struct_idxs),length(model_names),length(num_sample_points),length(sigmas));
err_H1 = zeros(length(eig_idxs),length(struct_idxs),length(model_names),length(num_sample_points),length(sigmas));
        ...
        for struct_idx_idx = 1:length(struct_idxs)
            ...
            for model_idx = model_idxs
                ...
                if strcmp(model_name,'GPR')
                   model_options.kfcn = kfcns{1};
                   model_options.sample_points = sampling_order_data.sampling_orders(:,:,eig_idx); % the eig_idx_idx is wrong here, should align the eig_idx within this script with the eig_idx in sampling_order_data
...
if isSaveData
    ...
    save('error_analysis_data',...
        ...
        'eig_idxs','struct_idxs','model_names','num_sample_points','sigmas',...
